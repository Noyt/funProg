...
abstract class TweetSet extends TweetSetInterface {
  ...
  def descendingByRetweet: TweetList = {
    ...
    def proc(set: TweetSet, list: TweetList): TweetList = {
      if(set.isEmpty) list
      else {
        val m = set.mostRetweeted
        proc(set remove m, new Cons(m, list))
      }
    }

    //TODO propre comme méthode ?
    def zec(set: TweetSet): TweetList = {
      if(set.isEmpty) Nil
      else{
        val m = set.mostRetweeted
        new Cons(m, zec(set remove m))
      }
    } 
