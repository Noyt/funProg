...
class TweetSetSuite {
  ...
  @Test def `filter: on all tweets, with 321 retweets`: Unit =
    new TestSets {
      val all = allTweets
      assertEquals(1, size(all.filter(tw => tw.retweets == 321)))
    }
  
  @Test def `just make allTweets`: Unit =
    new TestSets {
      val all = allTweets
    }
  ...
  @Test def `mostRetweeted: set 5`: Unit = 
    new TestSets {
      val m = set5.mostRetweeted
      assert(m.user == "a" || m.user == "b")
    }
