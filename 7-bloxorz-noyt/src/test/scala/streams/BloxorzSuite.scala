...
class BloxorzSuite {
  ...
  @Test def `neighbors history` : Unit =
    new Level1 {
      assertEquals(
        Set(
  ...
  @Test def `newNeighbors Only` : Unit = 
    new Level1 {
      assertEquals(
        Set(
          (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
  ...
  @Ignore
  @Test def `au bon test` : Unit =
  new Level1 {
    print(neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)))
  }

  @Test def `isLegal works` : Unit =
  new Level1 {
    val pos = Pos(0, 0)
    val blk = Block(pos, pos)

    assert(blk.isLegal)
  }

  @Test def `legalNeighbors works` : Unit = 
  new Level1 {
    val pos = Pos(2, 4)
    val blk = Block(pos, pos)

    println(blk.legalNeighbors)
  }
