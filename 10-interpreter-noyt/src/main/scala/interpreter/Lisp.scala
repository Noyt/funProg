...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'case :: expr_scrut :: xs => {
      xs match {
        case (els :: exp_els :: Nil) :: Nil => eval(exp_els, env)
        case (val1 :: exp1 :: Nil) :: ys => eval('if ::
        ('= :: expr_scrut :: val1 :: Nil) ::
        exp1 ::
        ('case :: expr_scrut :: ys) :: Nil, env)
      }
    }
    ...
    case 'def :: (Symbol(name) :: params) :: body :: expr :: Nil =>
      if (env == globalEnv)
        globalEnv = env.extendRec(name, env1 => eval(
          'lambda :: params :: body :: Nil
          , env1))
      eval(expr, env.extendRec(name, env1 => eval(
          'lambda :: params :: body :: Nil
          , env1)))
